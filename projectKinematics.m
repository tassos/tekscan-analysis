function [dist1, dist2] = projectKinematics (measPathName, threshold)
    ...
    rotations(:,:) = datamatrixtosave(:,38:40); %#ok<*NODEF> It is loaded on the line above
    translations(:,:) = datamatrixtosave(:,68:70);
...
%     [F_Tal, V_Tal] = STL_ReadFile(toLoadTal, 1, 'Select TALUS stl-file', Inf, 0, 1);
    ...
    V_Tib_New = (M*V_Tib_New'+repmat(translations(1,:)',1,size(V_Tib,1))-repmat(TranslationMatrixStlBone_Tib,1,size(V_Tib,1)))'*TransfoMatrixStlBone_Tib;
    ...
    if ~isempty(V_Tib_Cut)
        ...
        for i=1:length(rotations)
            ...
            V_Tib_New = (TransfoMatrixStlBone_Tib*V_Tib.'+TranslationMatrixStlBone_Tib*ones(1,size(V_Tib,1)))';
            V_Tib_New = (M*V_Tib_New'+repmat(translations(i,:)',1,size(V_Tib,1))-repmat(TranslationMatrixStlBone_Tib,1,size(V_Tib,1)))'*TransfoMatrixStlBone_Tib;
